...
impl Display for ParseError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Failed to parse into {}, with message: {}",
            self.to_parse_type, self.message
        )
    }
}
...
impl Display for RequestError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{} on endpoint {} with status {} and request data {}",
            self.message, self.endpoint, self.status, self.request_data
        )
    }
}
#[derive(Debug)]
